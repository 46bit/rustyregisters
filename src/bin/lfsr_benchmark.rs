...
fn main() {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    println!("{} took {:?}ms to perform 2^32 clockings.", Purple.bold().paint("PopCntLFSR"), pop_cnt_duration.num_milliseconds());

    let count_ones_duration = time(CountOnesLFSR::new(width, taps.clone(), vec![0]), seeds, clocks);
    println!("{} took {:?}ms to perform 2^32 clockings.", Purple.bold().paint("CountOnesLFSR"), count_ones_duration.num_milliseconds());

    let naive_duration = time(NaiveLFSR::new(width, taps.clone(), vec![0]), seeds, clocks);
    println!("{} took {:?}ms to perform 2^32 clockings.", Purple.bold().paint("NaiveLFSR"), naive_duration.num_milliseconds());
}
